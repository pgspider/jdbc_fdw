...
INSERT INTO text_tmp(c1) VALUES ('34.5');
EXPLAIN VERBOSE
SELECT pg_input_is_valid(c1, 'float8') FROM text_tmp;
SELECT pg_input_is_valid(c1, 'float8') FROM text_tmp;

DELETE FROM text_tmp;
INSERT INTO text_tmp(c1) VALUES ('xyz');
EXPLAIN VERBOSE
SELECT pg_input_is_valid(c1, 'float8') FROM text_tmp;
SELECT pg_input_is_valid(c1, 'float8') FROM text_tmp;

DELETE FROM text_tmp;
INSERT INTO text_tmp(c1) VALUES ('1e4000');
EXPLAIN VERBOSE
SELECT pg_input_is_valid(c1, 'float8') FROM text_tmp;
SELECT pg_input_is_valid(c1, 'float8') FROM text_tmp;
...
INSERT INTO text_tmp(c1) VALUES ('1e4000');
EXPLAIN VERBOSE
SELECT * FROM text_tmp, pg_input_error_info(text_tmp.c1, 'float8');
SELECT * FROM text_tmp, pg_input_error_info(text_tmp.c1, 'float8');
...
CREATE FOREIGN TABLE t (id serial OPTIONS(key 'true'), x float8 OPTIONS (column_name 'f1')) SERVER :DB_SERVERNAME OPTIONS (table_name 'float8_tmp_value');

INSERT INTO t(x) VALUES (float8 '-infinity');
INSERT INTO t(x) VALUES (-28), (-6), (-3.4), (-2.1), (-1.1), (-0.45),
                        (-1.2e-9), (-2.3e-13), (-1.2e-17), (0),
                        (1.2e-17), (2.3e-13), (1.2e-9),
                        (0.45), (1.1), (2.1), (3.4), (6), (28);
INSERT INTO t(x) VALUES (float8 'infinity'), (float8 'nan');
