...
--Testcase 904:
CREATE SERVER testserver1 FOREIGN DATA WRAPPER :DB_EXTENSIONNAME;
...
--Testcase 5:
CREATE USER MAPPING FOR CURRENT_USER SERVER :DB_SERVERNAME OPTIONS(username :DB_USER, password :DB_PASS);
--Testcase 6:
CREATE USER MAPPING FOR CURRENT_USER SERVER :DB_SERVERNAME2 OPTIONS(username :DB_USER, password :DB_PASS);
...
--Testcase 35:
SELECT c3, c4 FROM ft1 ORDER BY c3, c1 LIMIT 1;  -- ERROR
--Testcase 36:
...
ALTER EXTENSION :DB_EXTENSIONNAME ADD TEXT SEARCH CONFIGURATION public.custom_search;
...
ALTER EXTENSION :DB_EXTENSIONNAME DROP TEXT SEARCH CONFIGURATION public.custom_search;

--Testcase 889:
DROP TEXT SEARCH CONFIGURATION public.custom_search;

-- JDBC FDW does not support push down ORDER BY.
-- Therefore, the plan of these test cases is always not pushed down ORDER BY no whether the plan in Postgres FDW is pushed down or not.
-- ===================================================================
-- ORDER BY queries
-- ===================================================================
...
-- JDBC FDW does not support push down join. Therefore, plan of this test case is not support push down join.
-- SEMI JOIN
--Testcase 172:
EXPLAIN (VERBOSE, COSTS OFF)
...
--Testcase 897:
CREATE USER MAPPING FOR regress_view_owner_another SERVER :DB_SERVERNAME OPTIONS(username :DB_USER, password :DB_PASS);;
--Testcase 898:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM v4;
--Testcase 899:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM (SELECT * FROM v4 UNION ALL SELECT * FROM v4);
...
-- JDBC FDW does not support push down join.
-- Therefore, these test cases in the below section does not push down semi-join.
-- ===================================================================
...
-- change (bug #17889)
--Testcase 934:
CREATE FOREIGN TABLE loct1 (c1 int OPTIONS (key 'true')) SERVER :DB_SERVERNAME OPTIONS (table_name 'loct1_rescan');
--Testcase 935:
CREATE FOREIGN TABLE loct2 (c1 int OPTIONS (key 'true'), c2 text) SERVER :DB_SERVERNAME OPTIONS (table_name 'loct2_rescan');
--Testcase 936:
INSERT INTO loct1 (c1) VALUES (1001);
--Testcase 937:
INSERT INTO loct1 (c1) VALUES (1002);
--Testcase 938:
INSERT INTO loct2 (c1, c2) SELECT id, to_char(id, 'FM0000') FROM generate_series(1, 1000) id;
--Testcase 939:
INSERT INTO loct2 (c1, c2) VALUES (1001, 'foo');
--Testcase 940:
INSERT INTO loct2 (c1, c2) VALUES (1002, 'bar');
--Testcase 941:
CREATE FOREIGN TABLE remt2 (c1 int OPTIONS (key 'true'), c2 text) SERVER :DB_SERVERNAME OPTIONS (table_name 'loct2_rescan');
ANALYZE loct1;
ANALYZE remt2;
...
UPDATE remt2 SET c2 = remt2.c2 || remt2.c2 FROM loct1 WHERE loct1.c1 = remt2.c1;
--Testcase 947:
SELECT remt2.* FROM loct1, remt2 WHERE loct1.c1 = remt2.c1;
...
-- jdbc_fdw does not support batch insert, so the test below is commented out.
...
-- jdbc_fdw does not support truncate.
-- truncate rem1;
...
-- jdbc_fdw does not support query cancel
